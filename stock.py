...
class Product:
    ...
    def __init__(self, window):
        ...
        # Nombre de prodcto para realizar busqueda
        Label(frame, text = 'Nombre: ').grid(row = 4, column = 2)
        self.name_search = Entry(frame)
        self.name_search.grid(row = 4, column = 3)
    ...
    def add_product(self):
        if(self.price.get() != "" and self.amount.get() != ""):
            try:
                if (not(type(int(self.price.get())) is int)):
                    return
            except (ValueError):
                try:
                    if (not(type(float(self.price.get())) is float)):
                        return
                except (ValueError):
                    self.message_error['text'] = 'El precio tiene que ser un numero !'
                    return
            try:
                if(int(self.price.get()) <= 0):
                    self.message_error['text'] = 'El precio tiene que ser un numero positivo !'
                    return
            except (ValueError):
                if(float(self.price.get()) <= 0):
                    ...
                    return
            try:
                if (not(type(int(self.amount.get())) is int)):
                    self.message_error['text'] = 'La cantidad tiene que ser un numero entero !'
                    return
            except (ValueError):
                self.message_error['text'] = 'La cantidad tiene que ser un numero entero !'
                return
            if(int(self.amount.get()) <= 0):
                self.message_error['text'] = 'La cantidad tiene que ser un numero positivo'
                return
        if self.validation():
    ...
    def sell_product(self, name_sell, cantidad):
        try:
            if (cantidad != "" and not(type(int(cantidad)) is int)):
                return
        except (ValueError):
            if(cantidad != ""):
                self.message_error['text'] = 'La cantidad tiene que ser un numero entero !'
                    ...
                    if(row[1] == name_sell):
                        if (row[3] <= 0):
                            ...
                            if(row[3] - int(cantidad) >= 0):
                                ...
                                self.message['text'] = 'La cantidad maxima disponible para vender de {} es {}'.format(name_sell, row[3])
                            return
